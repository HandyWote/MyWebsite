 ...
 * - 桌面 project：纸面挂载画板，pointer 可绘制、草稿落盘、刷新恢复、撤销重做
 * - 非桌面 project：查看模式占位，纸面不挂载游戏
 *
...
/** 相机持续运动，locator.click() 的元素稳定检查必然超时；且纸面倾斜时
 *  按钮包围盒中心可能落在自身四边形之外——先探测验证点再发鼠标事件。 */
async function clickAt(page: Page, locator: Locator): Promise<void> {
	...
	await handle!.dispose();
	expect(point, "clickAt must find a verified point on the element").not.toBeNull();
...
test("desktop mounts the drawing board on the paper: draw, draft, reload, undo", async ({
	...
	const continueButton = boardAfterReload.locator('[data-draft-action="continue"]');
	await expect(continueButton).toBeVisible();
	await clickAt(page, continueButton);
