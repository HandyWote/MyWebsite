...
describe("sitemap prewarm script", () => {
	it("parses XML locations and warms every same-origin URL", async () => {
      ...
      <url><loc>https://example.com/articles/1?x=1&amp;y=2</loc></url>
		...
		await expect(
			prewarmSitemap("https://example.com/base", {
				fetchImpl: fetchMock,
				concurrency: 2,
			}),
		).resolves.toEqual({
	...
	it("fails when sitemap fetch or a warm request fails", async () => {
		await expect(
			prewarmSitemap("https://example.com", {
		...
		const fetchMock = vi
			...
			.mockResolvedValueOnce(
				response(
					"<urlset><url><loc>https://example.com/articles/1</loc></url></urlset>",
				),
			)
			.mockResolvedValueOnce(response("", false, 500));
		await expect(
			prewarmSitemap("https://example.com", { fetchImpl: fetchMock }),
		).rejects.toThrow("Prewarm request failed (500)");
	});

	it("waits for every warmed response body to finish", async () => {
		let releaseBody;
		const bodyFinished = new Promise((resolve) => {
			releaseBody = resolve;
		});
		const pageResponse = response();
		pageResponse.text.mockReturnValue(bodyFinished);
		const fetchMock = vi
			.fn()
			.mockResolvedValueOnce(
				response(
		...
		let settled = false;
		const warming = prewarmSitemap("https://example.com", {
			fetchImpl: fetchMock,
		}).finally(() => {
			settled = true;
		});
		await vi.waitFor(() => expect(pageResponse.text).toHaveBeenCalledOnce());
	...
	it("rejects invalid and cross-origin sitemap entries", async () => {
		await expect(
			prewarmSitemap("not a URL", { fetchImpl: vi.fn() }),
		).rejects.toThrow("Invalid sitemap URL: not a URL");

		const invalidLocation = vi
			.fn()
			.mockResolvedValue(
				response("<urlset><url><loc>not a URL</loc></url></urlset>"),
			);
		await expect(
			prewarmSitemap("https://example.com", { fetchImpl: invalidLocation }),
		).rejects.toThrow("Invalid sitemap URL: not a URL");
		...
		await expect(
			prewarmSitemap("https://example.com", { fetchImpl: crossOrigin }),
