...
export async function prewarmSitemap(siteUrl, options = {}) {
	...
	if (external)
		throw new Error(`Sitemap URL is outside ${origin}: ${external.toString()}`);
...
async function main() {
	...
	if (!siteUrl)
		throw new Error("PUBLIC_SITE_URL or a site URL argument is required");
	const result = await prewarmSitemap(siteUrl);
	process.stdout.write(
		`Warmed ${result.warmed} URLs from ${result.sitemapUrl}\n`,
	);
}
