...
describe('SEO site helpers', () => {
  ...
  it('builds canonical metadata and absolute media URLs only from PUBLIC_SITE_URL', () => {
    ...
    expect(metadata.alternates?.canonical).toBe('https://portfolio.example/articles/7');
    expect(metadata.description).toBe('A useful summary');
    expect(metadata.openGraph).toMatchObject({
      type: 'article',
      url: 'https://portfolio.example/articles/7',
      images: [{ url: 'https://portfolio.example/uploads/cover.webp', alt: 'Safe metadata' }],
    });
    expect(metadata.twitter).toMatchObject({
      card: 'summary_large_image',
      images: ['https://portfolio.example/uploads/cover.webp'],
    });
    expect(absoluteSiteUrl('https://cdn.example/media.webp')).toBe('https://cdn.example/media.webp');
  });

  it('creates stable PDF links and a plain-text description fallback', () => {
    process.env.PUBLIC_SITE_URL = 'https://portfolio.example';
    expect(getArticlePdfUrl({ pdf_filename: 'paper name.pdf' })).toBe('https://portfolio.example/api/articles/pdf/paper%20name.pdf');
    expect(summarizeArticle({ content: '# Heading\n\n[Visible](https://example.com) `hidden`' })).toBe('Heading Visible');
  });

  it('serializes Article JSON-LD without a script-closing injection', () => {
    process.env.PUBLIC_SITE_URL = 'https://portfolio.example';
    const malicious = { ...article, title: '</script><script>alert(1)</script>' };
