...
export function summarizeArticle(article: Pick<Article, 'summary' | 'content'>): string {
...
export function getArticleCoverUrl(article: Pick<Article, 'cover' | 'cover_url'>): string {
  return absoluteSiteUrl(article.cover_url || article.cover || '');
}

export function getArticlePdfUrl(article: Pick<Article, 'pdf_filename' | 'pdf_url'>): string {
  const pdfPath = article.pdf_url
    || (article.pdf_filename ? API_ENDPOINTS.PUBLIC.ARTICLE_PDF(article.pdf_filename) : '');
...
export function pageMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      title,
      description,
      images: absoluteImage ? [{ url: absoluteImage, alt: SITE_NAME }] : undefined,
    },
    twitter: {
