...
const assertColorMain = (colorPath: string, value: string) => {
	if (typeof value !== "string" || value.length === 0) {
		throw new Error(`[PixelTheme] ${colorPath} must be a non-empty string`);
	}
};

export const createPixelThemeOptions = (): ThemeOptions => {
	const palette = {
		mode: "dark" as const,
		primary: {
			main: colors.accent.blue,
			light: colors.accent.blueBright,
			dark: colors.accent.blueDim,
		},
		secondary: {
			main: colors.accent.blueBright,
		},
		error: {
