...
const { serverRequestMock } = vi.hoisted(() => ({
	serverRequestMock: vi.fn(),
}));
	...
	it("uses explicit 24 hour list, detail, sitemap, site-block, and profile policies", async () => {
		...
		expect(serverRequestMock.mock.calls).toEqual([
			...
			[
				...
				{
					cache: "force-cache",
					next: { revalidate: 86400, tags: ["article:42"] },
				},
			],
			[
				"/api/articles?page=3&per_page=100",
				{
					cache: "force-cache",
					next: { revalidate: 86400, tags: ["articles:list", "sitemap"] },
				},
			],
			[
				"/api/site-blocks",
				{
					cache: "force-cache",
					next: { revalidate: 86400, tags: ["site-blocks"] },
				},
			],
			[
				"/api/avatars",
				{
					cache: "force-cache",
					next: { revalidate: 86400, tags: ["profile"] },
				},
			],
		]);
	});
