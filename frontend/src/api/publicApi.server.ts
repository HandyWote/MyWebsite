...
function cachedServerRequest<T>(
	endpoint: string,
	...
	try {
		...
		return {
			...
			avatarUrl: currentAvatar
				? API_ENDPOINTS.PUBLIC.AVATAR_FILE(currentAvatar.filename)
				: DEFAULT_AVATAR,
		};
	} catch {
		return {
...
export async function getSitemapArticlePage(
	page = 1,
	perPage = 100,
): Promise<ArticlePage> {
	return cachedServerRequest<ArticlePage>(
...
async function fetchGitHubRepoPage(
	username: string,
	sort: string,
	perPage: number,
	page: number,
...
export async function getProjects() {
	...
	const username = String(
		config.github_username || SITE_BLOCK_DEFAULTS.projects_page.github_username,
	);
