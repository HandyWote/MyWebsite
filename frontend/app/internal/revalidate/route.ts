...
const ALLOWED_ACTIONS: Record<RevalidationEvent['entity'], ReadonlySet<string>> = {
...
function parseEvent(value: unknown): RevalidationEvent | null {
  ...
  if (typeof input.entity !== 'string' || typeof input.action !== 'string') return null;
...
export function planRevalidation(event: RevalidationEvent): RevalidationPlan {
  ...
  const detailTags = event.action === 'create' ? [] : event.ids.map(articleCacheTag);
  return {
...
export async function POST(request: Request) {
  ...
  if (!event) {
    return NextResponse.json({ ok: false, error: 'Invalid revalidation event' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, event, error: 'Revalidation failed' }, { status: 500 });
  }
