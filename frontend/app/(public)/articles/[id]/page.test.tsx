...
vi.mock('@/seo/data.server', () => ({ getArticleForPage: getArticleForPageMock }));
...
vi.mock('@/components/public/ArticleActions', () => ({ ArticleActions: () => <div data-article-actions /> }));
vi.mock('@/components/public/CommentSectionClient', () => ({ CommentSectionClient: () => <div data-comments /> }));
vi.mock('@/components/public/PdfViewerClient', () => ({
  PdfViewerClient: ({ filename, url }: { filename: string; url: string }) => <div data-pdf-viewer={filename} data-pdf-url={url} />,
}));
...
describe('article page SEO rendering', () => {
  ...
  it('generates canonical Article metadata with absolute media', async () => {
    ...
    await expect(generateMetadata({ params: Promise.resolve({ id: '7' }) })).resolves.toMatchObject({
      ...
      openGraph: { type: 'article', images: [{ url: 'https://portfolio.example/uploads/cover.webp' }] },
      twitter: { card: 'summary_large_image', images: ['https://portfolio.example/uploads/cover.webp'] },
    });
  });
  ...
  it('server-renders PDF summary, taxonomy and a stable link before enhancing the viewer', async () => {
    ...
    expect(html).toContain('href="https://portfolio.example/uploads/articles/pdfs/paper.pdf"');
    expect(html).toContain('data-pdf-url="https://portfolio.example/uploads/articles/pdfs/paper.pdf"');
  });
  ...
  it('turns a missing backend article into the Next not-found response', async () => {
    ...
    await expect(ArticlePage({ params: Promise.resolve({ id: '404' }) })).rejects.toThrow('404');
