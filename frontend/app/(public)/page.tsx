  ...
  return (
    <HomeEnterBoundary>
      <Box sx={{ height: 'calc(100dvh - 24px)', display: 'grid', gridTemplateRows: 'minmax(0, 1fr) auto', border: 1, borderColor: 'divider', overflow: 'hidden' }}>
        <Box sx={{ overflow: 'auto', display: 'flex', alignItems: { xs: 'flex-start', md: 'center' }, justifyContent: 'center', px: { xs: 1.25, sm: 4 }, py: { xs: 2, sm: 5 } }}>
          <Box sx={{ width: 'min(100%, 820px)', textAlign: 'center' }}>
            <Typography component="div" sx={{ color: 'text.disabled', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem', mb: 2 }}>~/intro.md - double click to enter articles</Typography>
            <Image src={avatarUrl} alt="avatar" width={84} height={84} priority className="home-avatar" />
            <Typography component="h1" sx={{ mt: 2, mb: 1, color: 'text.primary', fontFamily: 'JetBrains Mono, monospace', fontSize: { xs: '1.875rem', sm: '2.5rem' }, fontWeight: 700 }}>{String(home.title)}</Typography>
            <Typography component="p" sx={{ color: 'text.secondary', fontFamily: 'JetBrains Mono, monospace', mb: 2 }}>$ {String(home.subtitle)}</Typography>
            <Box component="nav" aria-label="Public pages" sx={{ display: 'flex', justifyContent: 'center', gap: 2, mb: 3, fontFamily: 'JetBrains Mono, monospace', fontSize: '0.875rem' }}>
              <Link href="/articles">articles/</Link>
              <Link href="/projects">projects/</Link>
            </Box>
            <Box sx={{ display: 'grid', gap: 1.5, color: 'text.secondary', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8125rem' }}>
              {socialLinks.length > 0 && <Box>{socialLinks.map((item, index) => <Link href={item.href || '#'} key={`${item.label}-${index}`} className="home-social-link">{item.label || item.value}</Link>)}</Box>}
        ...
        <TerminalCommandBar cwd="~/app" commands={['cd articles/', 'cd projects/', 'help']} />
      </Box>
    </HomeEnterBoundary>
  );
}
