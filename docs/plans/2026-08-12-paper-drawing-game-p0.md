...
- **日期**：2026-08-12
- **状态**：P0/P1（login 部分）/P2 已实施；M3D、P3、S3 与 P1 公共画布/审核链未做（逐项状态见「任务队列」与「后续计划引用」）
...
6. **匿名可画**：未登录可画可存 localStorage 草稿，不上传；只有登录用户可提交到公共画布。评论同理（P2，子计划已实施：见 [2026-08-16-p2-comment-account.md](./2026-08-16-p2-comment-account.md)）。
...
10. **任务队列**：P0（本计划）→ P1（login/用户/画布/审核）→ M3D（纸堆交互）→ P2（评论账号化，已合并 PR #7）→ P3（/members 成员页）→ S3 独立项。当前状态：P0 ✅（PR #6，2026-08-15）、P1 login 部分 ✅（PR #5，2026-08-14）、P2 ✅（PR #7，2026-08-16）；M3D / P3 / S3 ❌ 未做；P1 公共画布与审核链 ❌ 未做。
...
- **P1**：login 命令体系（TerminalCommandBar 掩码输入态 + GitHub OAuth + `-u` env 凭据）、users 表、公共画布版本化（drawing_versions 全量快照 + base_version 乐观锁）、四层审核链（TextDetector 粗筛 → ASCII 96×72 → OCR → 人工 pending）、限流
  - **✅ 已做**：GitHub OAuth 登录 + login 命令体系 + 限流 + admin 登录页移除 → [2026-08-12-p1-github-oauth-login.md](./2026-08-12-p1-github-oauth-login.md)（PR #5，2026-08-14 合并）
  - **❌ 未做**：公共画布版本化（drawing_versions + base_version 乐观锁）、四层审核链（TextDetector → ASCII 96×72 → OCR → 人工 pending）——P1 后续独立计划
- **M3D**：空白纸堆抽卡交互（冻结稿 v2.0，见上文第 3 条），技术栈后定 —— **❌ 未做**
- **P2**：评论账号化（草稿模式 + login 提醒 + GitHub 头像昵称）
  - **✅ 已做**：登录门槛 + 登录提醒 + 草稿模式（含移除手填昵称、admin 可评）→ [2026-08-16-p2-comment-account.md](./2026-08-16-p2-comment-account.md)（PR #7，2026-08-16 合并）
- **P3**：/members 成员页（GitHub 链接 + SEO）—— **❌ 未做**
- **S3 独立项**：ImportMarkdown base64 提取上传重写 —— **❌ 未做**
